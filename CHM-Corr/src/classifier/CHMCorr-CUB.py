...
from Utils import (
    ...
    iNaturalistPairedLayer4Extractor,
...
class CHMGridTransfer:
    def __init__(
        ...
        self.layer_4_extractor = layer_4_extractor
    ...
    def build(self):
        ...
        test_dl = DataLoader(test_ds, batch_size=self.BS, shuffle=False)
        self.compule_embeddings(test_dl)
    ...
    def compule_embeddings(self, test_dl):
        ...
        with torch.no_grad():
            for idx, batch in enumerate(test_dl):
                AE, BE = paired_extractor(
                    (batch["src_img"].cuda(), batch["trg_img"].cuda())
                )

                source_embeddings.append(AE)
                target_embeddings.append(BE)
...
def main():
    ...
    extractor = None
    if args.model == "inat":
        extractor = iNaturalistPairedLayer4Extractor
    elif args.model == "resnet50":
        extractor = PairedLayer4Extractor
