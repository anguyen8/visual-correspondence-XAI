...
parser.add_argument(
    "--target", type=int, default=859, help="The target class: 859 == toaster"
)
parser.add_argument(
    "--conf_target",
    type=float,
...
parser.add_argument(
    "--train_size", type=int, default=20000, help="Number of training images"
)
parser.add_argument(
    "--test_size", type=int, default=10000, help="Number of test images"
)
...
parser.add_argument("--netClassifier", default="resnet50", help="The target classifier")

parser.add_argument(
    "--outf", default="./logs", help="folder to output images and model checkpoints"
)
...
), "Traing set size + Test set size > Total dataset size"
...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(
        "/home/giang/Downloads/imagenet1k-val",
        transforms.Compose(
            [
...
wnid_to_id = train_loader.dataset.class_to_idx
id_to_wnid = dict((v, k) for k, v in wnid_to_id.items())


def train(epoch, patch, patch_shape):
    check_and_rm("{}".format(opt.outf))
    check_and_mkdir("{}".format(opt.outf))
    netClassifier.eval()
    success = 0
    total = 0
    recover_time = 0
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        # compute adversarial examples on examples that are originally classified correctly
        ...
        if adv_label == target:
            success += 1
            wnid = id_to_wnid[ori_label.cpu().item()]
            check_and_mkdir("{}/{}".format(opt.outf, wnid))
            # breakpoint()
            if plot_all == 1:
                # plot source image
                # vutils.save_image(data.data, "./{}/{}/{}_{}_original.jpeg".format(opt.outf, wnid, batch_idx, ori_label), normalize=True)

                # plot adversarial image
                vutils.save_image(
                    adv_x.data,
                    "./{}/{}/{}_{}_adversarial.jpeg".format(
                        opt.outf, wnid, batch_idx, adv_label
                    ),
                    normalize=True,
                )
...
def test(epoch, patch, patch_shape):
    ...
    for batch_idx, (data, labels) in enumerate(test_loader):
        ...
        # log to file
        progress_bar(
            batch_idx, len(test_loader), "Test Success: {:.3f}".format(success / total)
        )
